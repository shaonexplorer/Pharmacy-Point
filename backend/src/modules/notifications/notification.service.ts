...
export async function sendBatchAlert(recipient: string, type: 'low_stock' | 'expiry_approaching' | 'expired', items: { name: string; qty?: number; days?: number; threshold?: number }[]) {
  if (!process.env.SMTP_USER) throw new AppError(500, 'SMTP not configured');
